...
async def test_health(asgi_transport):
    async with AsyncClient(transport=asgi_transport, base_url="http://testserver", follow_redirects=True) as httpx_client:
...
async def test_auth_register_login_and_protected(asgi_transport):
    async with AsyncClient(transport=asgi_transport, base_url="http://testserver", follow_redirects=True) as httpx_client:
        ...
        r = await client.get_async_httpx_client().post("/api/v1/auth/register", json=reg_payload)
        assert r.status_code in (201, 400)

        l = await client.get_async_httpx_client().post("/api/v1/auth/login", json=reg_payload)
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://testserver", headers={"Authorization": f"Bearer {token}"}, follow_redirects=True) as auth_httpx:
    ...
    # Use AuthenticatedClient to call protected endpoint
    auth = AuthenticatedClient(base_url="http://testserver", token=token)
    auth_httpx = AsyncClient(transport=ASGITransport(app=app), base_url="http://testserver", headers={"Authorization": f"Bearer {token}"}, follow_redirects=True)
    auth.set_async_httpx_client(auth_httpx)
